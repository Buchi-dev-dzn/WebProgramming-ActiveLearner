...
このドキュメントは、2026-08-07 時点の Docker 実装が Linux VM 上でどう並んでいるかと、未実装の将来候補を 1 枚で確認するためのものです。現行状態は `docker-compose.yml`、各設定、`fastapi/app/main.py`を正とします。

認証は統合アカウント方式で、新規登録時に会員種別を選択しません。`customer` / `seller` は `buyer` と `seller` の両権限として扱い、`admin` は独立した権限として維持します。商品には所有者IDを保持し、商品登録・在庫更新の認可と監査ログ記録をバックエンドで行います。
