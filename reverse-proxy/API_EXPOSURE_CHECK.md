...
> 文書種別: 2026-07-24 時点の公開範囲の説明と検証記録です。公開ルートの正本は `../waf/conf.d/default.conf`、中継仕様の正本は `conf.d/default.conf` です。実測結果は実行時点の記録です。

このメモは、現在の構成で Reverse Proxy がどのように FastAPI へ中継しているか、また FastAPI の内部アクセス方法がクライアントへ直接露出していないかを確認するためのものです。

## 結論

現在の構成では、クライアントが使う入口はホストの `443` だけです。`80` は公開していません。
