...
- `refresh` / `logout`にOrigin検証と二重送信CSRFトークンを追加
- refresh token再利用時にfamily全体を失効し、critical監査イベントを記録
- 認証APIに送信元IP・アカウント単位のアプリ内レート制限を追加
- FastAPI/NIDS/HIDSを非rootユーザー、read-only filesystem、capability削減で実行

## 未実装・本番導入前に必要

- `.env.example`の値を使わない秘密情報管理、鍵・DB資格情報・センサートークンのローテーション
- 複数インスタンス・複数入口で共有する分散rate limiter
- DBバックアップの暗号化、監査ログの改ざん耐性、保持期間・削除方針、監視通知
- NIDSの実パケット監視、HIDSのauditd/eBPF/AppArmor/SELinux等によるOSレベルの保護
- Nginx/HAProxyコンテナを含む全サービスの非root化、イメージ固定・脆弱性スキャン
